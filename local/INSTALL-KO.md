...
일반 논문 조사 스킬, 공개 UniPaper MCP, 로컬 KHU 1편 확보·읽기 도구, 로컬 Zotero
저장, 로컬 `audit_draft_claims`를 모두 연결합니다. 사용자가 KHU나 초안 검증
도구를 따로 지시할 필요는 없습니다.

Codex는 일반 학술검색과 합법적인 공개 원문을 먼저 시도합니다. 답변에
실제 원문이 필요한데 공개 원문을 읽을 수 없을 때만 KHU 전용 브라우저를
자동으로 엽니다. 헬퍼가 요청한 논문 PDF 1편을 로컬 임시폴더에 저장하고
검증한 뒤 Codex가 필요한 페이지를 읽어 원래 분석을 이어갑니다. 자동으로
PDF 버튼을 찾지 못할 때만 열린 창에서 그 버튼을 한 번 눌러야 할 수 있으며,
파일을 찾아 대화에 첨부할 필요는 없습니다. 분석과 Zotero 저장 뒤 임시파일은
자동 삭제됩니다.

논문 초안이나 학위논문의 인용을 확인해 달라고 하면 Codex가 문장을 개별
주장으로 나누고 읽은 원문의 DOI와 페이지·표·그림 위치를 연결합니다. 그 뒤
컴퓨터 안의 로컬 도구가 각 문장을 `SUPPORTED`, `PARTIAL`, `CONTRADICTED`,
`UNVERIFIED`로 검사합니다. 이 로컬 도구는 초안을 저장하거나 인터넷으로
보내지 않고, 파일·Zotero·학교 계정에도 접근하지 않습니다.

Zotero Desktop을 사용하는 사람은 앱을 설치하고 실행한 뒤 Codex에
“앞으로 연구에 실제로 쓰인 중요한 원문은 Zotero에 자동 저장해줘”라고 한
번 요청하면 됩니다. 이후 DOI로 중복을 확인하고 핵심 논문만 현재 선택된
Zotero 라이브러리나 컬렉션에 저장합니다. 공개 원문은 자동 첨부할 수
있고, 기관 구독 원문은 자신의 권한으로 로컬 헬퍼가 확보한 그 논문 1편의
검증된 임시파일만 Zotero에 첨부합니다.
